...
const BlogCard: React.FC<BlogCardProps> = (props) => {
  ...
  return (
    <article
      className={`group rounded-xl border border-white/5 bg-white/5 transition hover:translate-y-[-2px] hover:bg-white/10 ${
        featured ? "p-5 md:col-span-2 md:row-span-2 md:p-8" : "flex flex-col p-5"
      }`}
    >
      {/* Cover Image */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-slate-300 ${featured ? "text-sm" : "text-xs"}`}>
                {author.name.length > 13 ? `${author.name.substring(0, 10)}...` : author.name}
              </span>
            </div>
